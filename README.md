...
This generates `wallscape-2.5-Linux.deb` in the `build/` directory.
...
3. Commit and push to `main`:
   ```bash
   git commit -am "feat: release v2.5"
   git push origin main
   ```
4. GitHub Actions will automatically:
   * Build WallScape on Ubuntu
   * Package `wallscape-2.5-Linux.deb`
   * Extract release notes from `CHANGELOG.md`
   * Create the tag `v2.5` and publish the GitHub Release with the `.deb` attached
